import type { FailureCandidate, NormalizedLine, Parser } from "./types.js";

function candidate(
  line: NormalizedLine,
  values: Omit<FailureCandidate, "lineNumber">
): FailureCandidate {
...
export const eslintParser: Parser = {
  id: "eslint",
  parse(lines) {
    const results: FailureCandidate[] = [];
...
export const testParser: Parser = {
  id: "javascript-tests",
  parse(lines) {
    const results: FailureCandidate[] = [];
...
export const packageManagerParser: Parser = {
  id: "package-manager",
  parse(lines) {
    const results: FailureCandidate[] = [];
    for (const line of lines) {
...
export const pytestParser: Parser = {
  id: "pytest",
  parse(lines) {
    const results: FailureCandidate[] = [];
    for (const line of lines) {
...
export const builtInParsers: Parser[] = [
  typescriptParser,
  eslintParser,
  testParser,

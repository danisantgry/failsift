...
export function resolveLimits(limits: Partial<InputLimits> = {}): InputLimits {
  const resolved = {
...
export function measureInput(text: string, limits: InputLimits): LimitsApplied {
...
export async function readStreamLimited(stream: Readable, limits: InputLimits): Promise<string> {
  const chunks: Buffer[] = [];
  let bytesRead = 0;
...
export async function readInput(path: string, limits: InputLimits): Promise<string> {
  if (path === "-") return readStreamLimited(process.stdin, limits);
  let info;
  try {
  ...
  if (!info.isFile()) throw new InputError(`Input path is not a file: ${path}`);

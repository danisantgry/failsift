...
export async function initialize(options: InitOptions = {}): Promise<InitResult> {
  ...
  const workflows = normalizeWorkflowNames(
    options.workflows?.length ? options.workflows : await discoverWorkflows(root, outputPath)
  );

  if (workflows.length === 0) {
    throw new InputError(
      "No CI workflow was detected. Pass its exact name with --workflow, for example: failsift init --workflow CI"
    );
  }
  ...
  if (existing !== undefined && !options.force) {
    throw new InputError(
      `${relative(root, outputPath)} already exists and differs. Review it or rerun with --force to replace it.`
    );
  }
...
export function createWorkflow(workflows: string[]): string {
  const document = {
    name: "FailSift",
    on: {
      workflow_run: {
...
async function discoverWorkflows(root: string, outputPath: string): Promise<string[]> {
...
async function readCandidate(path: string): Promise<WorkflowCandidate | undefined> {
  ...
  if (!isRecord(document) || typeof document.name !== "string" || !isRecord(document.jobs)) return undefined;
  if (hasEvent(document.on, "workflow_run")) return undefined;

  const name = document.name.trim();
  if (!name || /failsift/iu.test(name)) return undefined;
  let score = 0;
  if (/^(ci|test|tests|build|checks?)$/iu.test(name)) score += 100;
  if (hasEvent(document.on, "pull_request")) score += 40;
  if (hasEvent(document.on, "push")) score += 20;
  if (/codeql|security|release|deploy|publish|dependabot/iu.test(name)) score -= 100;
...
function normalizeWorkflowNames(names: string[]): string[] {
...
function hasEvent(value: unknown, event: string): boolean {

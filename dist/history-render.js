...
export function renderHistoryReport(report, format) {
    if (format === "json")
        return `${JSON.stringify(report, null, 2)}\n`;
    if (format === "markdown")
        return renderHistoryMarkdown(report);
    return renderHistoryTerminal(report);
}
export function renderHistoryTerminal(report) {
    const lines = [
    ...
    if (report.failureGroups.length === 0) {
        ...
        if (omitted > 0)
            lines.push(`... ${omitted} additional one-off group${omitted === 1 ? "" : "s"} omitted.`);
    }
    lines.push("", `Unique fingerprints: ${report.uniqueFingerprints}`, `Recurring fingerprints: ${report.recurringFingerprints}`, `Unclassified runs: ${report.unclassifiedRuns.length}`, `Redactions: ${report.redactionCount}`);
    return `${lines.join("\n")}\n`;
}
export function renderHistoryMarkdown(report) {
    const lines = [
        "## FailSift failure history",
        "",
    ...
    if (report.failureGroups.length === 0) {
        ...
        const omitted = report.failureGroups.length - DISPLAY_LIMIT;
        if (omitted > 0)
            lines.push("", `_${omitted} additional one-off group${omitted === 1 ? "" : "s"} omitted from this view._`);
    }
    lines.push("", `**Unique fingerprints:** ${report.uniqueFingerprints}  `, `**Recurring fingerprints:** ${report.recurringFingerprints}  `, `**Unclassified runs:** ${report.unclassifiedRuns.length}  `, `**Redactions:** ${report.redactionCount}`, "", "_FailSift analyzes bounded, redacted logs as untrusted text and never executes their contents._", "");
    return lines.join("\n");
}
